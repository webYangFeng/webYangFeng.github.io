...
let { domName } = defineProps(["domName"]);
function init() {
  let main = document.querySelector("." + domName);
  const maxW = main.offsetWidth;
  const maxH = main.offsetHeight;
  const maxSize = 5;
  const minSize = 1;
  let isMoving = false;
  let timer = null;

  let canvas = document.getElementById("hoverCanvas");
  canvas.width = maxW;
  canvas.height = maxH;
  ...
  Star.prototype = {
    ...
    link() {
      ...
      ctx.strokeStyle = "rgba(100, 150, 255, 0.125)";
      ctx.stroke();
      ctx.restore();
    },
    cache() {
